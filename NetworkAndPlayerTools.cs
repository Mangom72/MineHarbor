...
internal static partial class Launcher
{
	...
	private static Label CreateToolLabel(string text, float size, bool bold)
	{
		...
		label.Text = text;
		label.AutoSize = true;
		label.Font = new Font("Segoe UI Variable Text", size, bold ? FontStyle.Bold : FontStyle.Regular);
		return label;
	}
	...
	private static TextBox CreateToolValueBox()
	{
		...
		box.Font = new Font("Segoe UI Variable Text", 10F, FontStyle.Bold);
		box.TabStop = false;
		return box;
	}
	...
	private sealed class PlayerManagementForm : Form
	{
		...
		public PlayerManagementForm(Func<string, bool> commandSender)
		{
			...
			playerNameBox.Font = new Font("Segoe UI Variable Text", 10F);
	...
	private sealed class NetworkToolsForm : Form
	{
		...
		public NetworkToolsForm(string profileServerDirectory, int port, string selectedJavaPath, Action externalRecheckAction, string initialPublicIp, bool detectedCgnat, bool needsFirewallCheck, string upnpResult)
		{
			...
			actionGrid.ColumnCount = 5;
			actionGrid.RowCount = 1;
			for (int column = 0; column < 5; column++)
			{
				actionGrid.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 20F));
			}
			...
			actionGrid.Controls.Add(playitButton, 4, 0);
			...
			guideBox.Font = new Font("Segoe UI Variable Text", 9F);
		...
		private void ApplyLanguage()
		{
			...
			routerButton.Text = ToolText("공유기 페이지", "Router page");
			playitButton.Text = ToolText("playit.gg 안내", "playit.gg guide");
