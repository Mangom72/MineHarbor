...
internal static partial class Launcher
{
	...
	// IPv6 주소를 가리는 공통 패턴입니다. 운영 기록과 진단 번들이 같은 기준을 쓰도록 한곳에서 관리합니다.
	// 로그 시각(12:34:56)이나 Class::method 표기가 지워지지 않도록 압축 표기(::)를 쓰거나
	// 8개 그룹을 모두 갖춘 주소만 대상으로 합니다.
	private const string OperationIPv6AddressPattern =
	...
	private static string SanitizeOperationMessage(string value, string serverDirectory)
	{
		...
		// IPv6로 접속한 플레이어 주소도 IPv4와 동일하게 가립니다.
		text = Regex.Replace(text, OperationIPv6AddressPattern, "[IP]");
