...
[최신 EXE 다운로드](./releases/latest/download/Minecraft-Server-Launcher.exe)

고정 다운로드 파일명은 `Minecraft-Server-Launcher.exe`입니다. 이후 버전도 같은 링크로 갱신됩니다.
...
1. `Minecraft-Server-Launcher.exe`를 원하는 폴더에 둡니다.
2. EXE를 실행합니다.
...
메인 화면의 `서버 관리`에서 프로필 관리 또는 멀티 서버를 선택할 수 있습니다. 프로필 관리에서는 새 서버 생성, 복제, 기존 서버 폴더 가져오기, 이름 변경, 안전 보관을 할 수 있습니다. 프로필마다 별도 폴더가 만들어지므로 월드, 플러그인, 모드와 설정을 따로 유지할 수 있습니다.
...
[Download latest EXE](./releases/latest/download/Minecraft-Server-Launcher.exe)

The permanent download asset name is `Minecraft-Server-Launcher.exe`. Future releases update the same link.
...
1. Put `Minecraft-Server-Launcher.exe` in the folder where you want to run the server.
2. Run the EXE.
...
Use `Server management` to open either profile management or the multi-server dashboard. Profile management can create, clone, import, rename, activate, or safely archive servers. Each profile has its own folder, world, plugins, mods, and settings. The multi-server dashboard can run profiles on separate ports at the same time and stops crash loops after three failures in ten minutes.
