...
현재 제품 버전은 `v0.3.2`, 내부 빌드 번호는 `26.2.45.26`입니다.
...
사용자에게 표시되는 제품 버전은 Semantic Versioning 형식의 `v0.3.2`이고, `26.2.45.26`는 소스와 빌드를 구분하는 내부 번호입니다. 두 값의 단일 기준은 `version.json`입니다.
...
The current product version is `v0.3.2`; the internal build number is `26.2.45.26`.
...
The user-facing product version uses Semantic Versioning (`v0.3.2`); `26.2.45.26` is a separate internal build number. `version.json` is the single source for both values.
