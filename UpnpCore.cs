    ...
    internal class SocketUpnpPortMappingService : IPortMappingService
    {
        ...
        public async Task<UpnpMappingResult> MapPortsAsync(int externalPort, int internalPort, string internalIp, bool needUdp, string description, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                // TODO: Download XML, Parse, and SOAP
                result.Success = true;
            }
            ...
            try
            {
                ...
                foreach (string line in lines)
                {
                    ...
                    if (parts.Length >= 10)
                    {
                        ...
                        int ext; int.TryParse(parts[4], out ext); m.ExternalPort = ext;
                        int intP; int.TryParse(parts[5], out intP); m.InternalPort = intP;
                        m.InternalIp = parts[6];
                        m.Protocol = parts[7];
                        m.Description = parts[8];
                        long ticks; if (long.TryParse(parts[9], out ticks)) m.CreatedAt = new DateTime(ticks);
                        list.Add(m);
                    }
                }
            }
            ...
            try
            {
                ...
                foreach (var m in mappings)
                {
                    sb.AppendLine(string.Format("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\t{9}",
                        m.ProfileId, m.RouterId, m.ServiceType, m.ControlUrl, m.ExternalPort, m.InternalPort, m.InternalIp, m.Protocol, m.Description, m.CreatedAt.Ticks));
                }
