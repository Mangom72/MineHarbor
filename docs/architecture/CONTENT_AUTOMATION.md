...
일정 검사는 주 MineHarbor 창 또는 다중 서버 관리 창이 실행 중일 때 동작합니다. 이 버전은 Windows 서비스나 로그온 전 백그라운드 에이전트를 설치하지 않으므로 앱이 완전히 종료된 동안에는 새 작업을 시작하지 않습니다. 다음 실행 시 앱이 열리면 실행 시각이 지난 활성 작업을 최대 한 번만 청구합니다. 작업별로 다음 실행 때 한 번 실행, 최대 지연 이후 건너뛰기, 실행 없이 알림 기록 중 하나를 선택할 수 있으며 놓친 횟수만큼 무제한 실행하지 않습니다.
...
Per-server manifests distinguish managed and manual content, constrain all paths to the server root, and use validated atomic JSON replacement. Modrinth installation enforces exact game/loader compatibility, required dependencies, bounded HTTPS downloads, and hashes. While a MineHarbor management window is running, automation schema 2 supports interval, daily, selected-weekday, and one-time jobs, bounded missed-run policies, on-disk claims, expired-lease recovery, results, and count/day/size backup retention; schema 1 migrates in memory and no Windows background service is installed. Operations history keeps up to 500 local entries with cross-process locking, atomic writes, redaction, and a verified SHA-256 hash chain. Long work is cancellable, and the dashboard reports unsupported data instead of guessing.
