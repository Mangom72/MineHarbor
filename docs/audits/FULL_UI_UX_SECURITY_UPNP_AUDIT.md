...
## 24. v1.9.0 빠른 명령 호환성·위험도 후속 감사 (2026-07-25)

기존 빠른 명령은 48개 기본 항목과 `Confirm` 불리언을 사용했다. `QuickCommandRisk` 열거형은 선언되어 있었지만 실제 정의·목록 색상·전송 확인에는 연결되지 않았고, 템플릿은 정확히 같은 토큰 수만 허용해 Minecraft에서 생략 가능한 차단·추방 사유와 아이템 수량까지 반드시 입력해야 했다. 서버 종류만 검사하고 Minecraft 버전은 보지 않아 1.12 서버에 데이터팩 명령을 제안하거나 26.x에서 변경된 구형 시간 구문을 계속 보여 줄 가능성도 있었다.

제재 조회와 IP 차단, 시드·시간, 유휴 제한, 게임 규칙, 데이터팩 변경, `reload` 및 읽기 전용 고급 조회를 추가해 기본 목록을 70개 이상으로 확장했다. `{필수}`와 `[선택]` 인수를 분리하고 마지막 사유·메시지·명령은 여러 토큰을 받을 수 있게 했다. 사용자 명령은 최소·최대 Minecraft 버전과 `Normal`·`Confirm`·`Dangerous`를 저장하며, 기존 `Confirm` JSON은 자동 변환한다.

명령 선택창과 로컬 자동완성은 현재 서버 버전에 맞는 정의만 사용한다. Minecraft 공식 변경 기록에 맞춰 데이터팩과 강제 로딩은 1.13 이상, `playersSleepingPercentage`는 1.17 이상에서 노출하고, 기존 `time query daytime`은 1.21.11까지만 제공한다. `reload`, 광범위 변경과 위험한 게임 규칙 값은 빨간 강한 경고를 사용하고, 같은 루트의 `worldborder get`·`forceload query`는 읽기 전용으로 구분한다. 테스트는 선택 인수 생략·여러 단어 사유·잘못된 선택 인수 순서, 버전 경계, 신규 자동완성 후보, JSON 왕복, 목록 그룹과 조건부 위험도를 검증한다.

수정 사항은 `v1.9.0`으로 출시했다. PR과 `main` CI, 자체서명 릴리스 산출물 7종 검사가 모두 통과했으며, 공개 v1.8.1 런처가 게시된 메타데이터를 통해 v1.9.0으로 업데이트된 뒤 27개 테스트 그룹, Portable 실행·버전, 명령 브리지 10개, UI·보안 회귀 검사를 다시 통과했다.

## 25. v1.10.0 단계형 빠른 명령 작성 UX 후속 감사 (2026-07-26)

기존 입력은 명령 템플릿을 일반 문자열로 삽입해 `{gamemode}` 같은 플레이스홀더의 완료 상태를 구분하지 못했고, 후보를 적용한 뒤 다음 인수로 자연스럽게 이어지지 않았다. 빠른 명령 입력을 템플릿 문구·필수·선택·기본값 인수를 분리하는 상태 모델과 직접 그리는 인라인 토큰 컨트롤로 교체했다. 미완성은 회색, 현재 단계는 강조색, 잘못된 값은 위험색으로 표시하며 이미 확정한 다른 인수는 유지한다.

키보드 흐름은 위·아래 후보 이동, `Tab`/미완성 상태의 `Enter` 확정, `Shift+Tab` 이전 인수, `Esc` 목록 닫기·작성 취소, 필수 인수 완료 후 `Enter` 전송으로 정리했다. 선택 인수는 비워도 전송할 수 있지만 값을 입력했다면 해당 유형 검증을 통과해야 한다. 명령과 후보 선택은 실행을 유발하지 않고 위험 명령 확인은 최종 전송 시 완성된 실제 명령만 보여 준다.

기존 140px 빠른 명령 후보 목록은 항목과 창 공간에 맞춰 최대 430px까지 확장하고 입력 영역을 피하도록 위·아래 중 넓은 공간을 선택한다. 공통 플레이어·콘솔 자동완성도 최대 20개 후보와 380px 높이를 사용하며, 후보를 `Enter`로 확정한 이벤트가 뒤쪽 전송 처리까지 이어지지 않도록 소비한다.

회귀 검사는 필수·선택·기본값 파싱, 연속 단계 이동, 이전 단계 이동 후 값 유지, 아이템 부분 검색, 선택 인수 생략, 잘못된 인수에서 단계 유지, 목록의 가변 높이·비중첩 및 스크린 리더 정보를 포함한다. 실제 서버 명령과 외부 네트워크는 실행하지 않고 임시 데이터와 리플렉션 기반 Windows UI 검사만 사용한다.

수정 사항은 `v1.10.0`으로 출시했다. PR과 `main` CI, 자체서명 릴리스 워크플로가 모두 통과했으며 공개 자산 7종을 별도 폴더에 다시 내려받아 서명·SHA-256·메타데이터를 검증했다. 공개 v1.9.0 런처의 실제 업데이트 코드로 `PUBLIC_AUTO_UPDATE_OK=1.9.0->1.10.0`을 확인하고, 내려받은 v1.10.0 실행 파일로 27개 런처 테스트 그룹, 브리지 프로토콜 10개, Portable·UI·보안 검사를 다시 통과했다.

## 26. v1.11.0 예약 신뢰성·운영 기록 후속 감사 (2026-07-26)

제안된 통합 플랫폼 범위 중 기존 구조를 안전하게 확장할 수 있고 현재 운영 문제를 직접 줄이는 고급 예약과 로컬 운영 기록을 우선 적용했다. 별도 백그라운드 에이전트, 웹·Discord 원격 제어와 외부 저장소는 인증·프로세스 수명·권한 경계가 먼저 확정되어야 하므로 이번 릴리스에서 구현된 것처럼 표시하지 않았다.

자동화 스키마 2는 기존 반복 간격·매일 일정에 선택 요일과 특정 날짜 한 번을 추가한다. 기존 스키마 1 파일은 읽는 동안 메모리에서만 변환하며 사용자가 저장하기 전에는 원본을 바꾸지 않는다. 미래 스키마와 손상 파일도 보존한 채 거부한다. 작업별 최대 지연과 `다음 실행 때 한 번`, `건너뛰기`, `알림만 기록` 정책을 도입해 절전이나 앱 종료 뒤 오래된 작업이 무제한 따라잡기 실행되지 않게 했다. 일회성 일정은 디스크 임대를 얻는 순간 비활성화해 다음 검사에서 다시 청구하지 않는다.

예약 편집기는 저장 전에 다음 실행 시각, 작업 위험도, 서버가 꺼져 있을 때 처리와 5분 이내 다른 작업 충돌을 표시한다. 관리 도구는 새 운영 기록 진입점을 포함해 3×3으로 정리하고 최소 창 크기에서 한국어·영어 버튼 문구가 잘리지 않는지 검사한다.

서버별 `.mineharbor/operations-history.json`은 최대 500개와 4MiB로 제한하고 원자적 파일 교체, 프로세스 내부 잠금과 경로별 Windows 뮤텍스를 사용한다. 항목 식별자·시각·분류·중요도·출처·양언어 문구와 이전 해시를 SHA-256으로 연결하며, 읽음 상태는 감사 내용과 분리해 갱신할 수 있다. 절대 서버 경로·IPv4·토큰·비밀번호·웹훅 형태는 기록 전에 가린다. 해시 변조·손상·미래 스키마는 원본을 덮어쓰지 않고 화면에 검증 실패로 표시한다. 이 연속 해시는 우발적 변경 감지 수단이며 침해된 Windows 계정에 대한 전자서명이나 외부 증명은 아니다.

메인 운영 기록 화면은 서버·중요도·읽음 필터, 개별/전체 읽음, 새로고침과 표시 목록 CSV 내보내기를 지원한다. 서버 제어가 기록 저장 실패에 묶이지 않도록 기록 실패는 격리하며, 오류 로그에는 원시 경로나 비밀값 대신 예외 유형만 남긴다.

임시 서버 폴더와 가짜 루프백 UPnP만 사용한 로컬 검증에서 자동화 스키마 이전·미래/손상 보존·요일 계산·숫자 요일 거부·최대 지연 건너뛰기·일회성 중복 청구 차단, 운영 기록 읽음 상태·경로/IPv4/비밀값 가림·해시 변조·미래 스키마·접근성을 포함한 28개 런처 테스트 그룹이 통과했다. Portable 버전·실행, Paper 브리지 10개, 모던 대화상자와 보안 정적 검사도 통과했다. 실제 사용자 서버·공유기·UPnP 매핑은 변경하지 않았고, 이 장치에는 .NET SDK가 없어 SDK 스타일 `net48` 빌드는 PR CI에서 검증한다.

수정 사항은 `v1.11.0`으로 출시했다. PR과 `main` CI에서 .NET 10 SDK의 경고=오류 `net48` 빌드와 전체 검사가 통과했고, 자체서명 릴리스 워크플로도 임시 인증서 제거와 공개 자산 7종을 검증했다. 공개 자산을 별도 폴더에 다시 내려받아 자체서명·SHA-256·메타데이터를 독립 확인했으며, 공개 v1.10.0 런처의 실제 업데이트 코드로 `PUBLIC_AUTO_UPDATE_OK=1.10.0->1.11.0`을 확인한 뒤 업데이트된 실행 파일로 28개 런처 테스트 그룹, 브리지 10개, Portable·UI·보안 검사를 다시 통과했다.

## 27. v1.12.0 백그라운드 운영 기반 후속 감사 (2026-07-26)

통합 플랫폼 계획 전체를 한 번에 노출하지 않고, GUI 종료 뒤 예약과 에이전트 소유 서버를 유지하는 데 필요한 최소 신뢰 경계를 우선 구현했다. 백그라운드 운영은 기본 비활성화이며 `서버 관리 → 백그라운드`에서 명시적으로 동의한 현재 사용자만 사용할 수 있다. 관리자 권한 서비스나 로그인 전 실행은 설치하지 않고, 선택한 경우에만 현재 사용자 Windows 자동 시작을 등록한다.

GUI와 에이전트는 별도 단일 인스턴스 뮤텍스로 함께 실행한다. 로컬 IPC는 사용자 SID 파생 이름과 해당 SID 전용 ACL의 이름 있는 파이프를 사용하고 JSON 요청 16KiB, 응답 256KiB, 수신 시간 3초를 상한으로 둔다. 임의 셸·실행 파일 호출은 제공하지 않는다. 트레이는 서버별 시작·안전 종료·재시작·즉시 백업·콘솔과 전체 안전 종료, 예약 일시 중지, 완전 종료를 제공한다. 콘솔은 기존 현대형 입력·자동완성과 3단계 위험 확인을 재사용한다.

프로세스 소유권은 포트만으로 추측하지 않는다. 에이전트가 직접 시작하고 표준 입출력을 보유한 `--managed-profile` 자식만 명령·종료·실행 중 백업하며, 같은 포트를 다른 창이나 프로세스가 사용하면 외부 소유로 표시하고 작업을 건너뛴다. 관리 자식은 부모 PID와 시작 시각을 검증하고 부모가 사라졌을 때 Java 서버에 `stop`을 먼저 전송한다. 정상 에이전트 종료와 런처 업데이트는 모든 소유 서버가 제한 시간 안에 종료된 뒤에만 계속하고, 그렇지 않으면 강제 종료 대신 취소한다.

자동화 파일은 기존 프로세스 내부 잠금 외에 서버 경로별 Windows 뮤텍스로 읽기-수정-쓰기를 직렬화했다. PID·시작 시각 임대와 결합해 GUI와 에이전트가 같은 작업을 중복 청구하지 않으며, 절전 복귀는 기존 최대 지연과 `run-once`·`skip`·`notify-only` 정책만 다시 평가한다. 에이전트 소유 서버의 백업은 `save-off → save-all flush → 백업 → save-on`을 사용하고, 소유하지 않은 실행 중 서버는 실패 기록만 남긴다.

UX 점검에서는 서버 관리 도구에 현재 연결 상태를 점으로 표시하는 백그라운드 진입점을 추가하고, 에이전트가 연결된 동안 시작·종료·콘솔을 같은 화면에서 위임한다. 충돌 재시작 체크박스는 중복된 상태를 만들지 않도록 에이전트 설정에서 관리된다는 문구로 바뀐다. 설정·콘솔 창은 한·영 문구, DPI, 키보드, 스크린 리더 이름, 기존 테마 입력·버튼을 사용한다.

임시 파일과 무작위 미사용 파이프 이름만 사용한 로컬 검증은 기본 비활성화, 설정 왕복, 손상·미래 스키마 원본 보존, 자동 시작 위임, 접근성과 기존 전체 기능을 포함한 29개 런처 테스트 그룹을 통과했다. Portable 버전·실행, Paper 브리지 10개, 현대 UI와 보안 정적 검사도 통과했다. 실제 사용자 서버, Windows 자동 시작, 공유기, UPnP와 외부 포트는 변경하지 않았다.

수정 사항은 `v1.12.0`으로 출시했다. PR과 `main` CI에서 .NET 10 SDK 경고=오류 `net48` 빌드와 전체 검사가 통과했고, 자체서명 릴리스 워크플로도 임시 인증서 제거와 공개 자산 7종을 검증했다. 공개 자산을 별도 폴더에 다시 내려받아 자체서명·SHA-256·메타데이터를 독립 확인했으며, 공개 v1.11.0 런처의 실제 업데이트 코드로 `PUBLIC_AUTO_UPDATE_OK=1.11.0->1.12.0`을 확인한 뒤 업데이트된 실행 파일로 29개 런처 테스트 그룹, 브리지 10개, Portable·UI·보안 검사를 다시 통과했다.

## 28. v1.13.0 선택형 Windows 알림 후속 감사 (2026-07-26)

기존 `운영 기록`은 서버별 사건을 안전하게 보관했지만 사용자가 화면을 직접 열기 전에는 실패를 알 수 없었다. 백그라운드 에이전트에는 트레이 아이콘과 일부 직접 작업 결과 알림이 있었으나, 예약·충돌·백업 사건과 사용자 설정을 연결하는 일관된 알림 경로는 없었다.

Windows 알림은 기본 비활성화 상태에서 별도 동의를 받은 경우에만 에이전트의 기존 `NotifyIcon`으로 표시한다. 새로운 네트워크 리스너, 외부 알림 공급자나 관리자 권한 서비스는 만들지 않았다. 정보·경고·오류 최소 중요도, 서버·예약·백업·콘텐츠·네트워크·업데이트/보안 종류와 자정을 지나는 조용한 시간을 설정할 수 있다.

에이전트가 처음 서버 기록을 볼 때 최신 항목을 기준점으로 삼아 이전 기록을 갑자기 다시 알리지 않는다. 보존 한도로 기준점이 사라져도 현재 최신 항목으로 이동한다. 새 사건은 최대 50개로 제한하고 8초 안에 여러 건이 발생하면 가장 중요한 최신 사건과 추가 개수를 하나로 요약해 WinForms 알림이 서로 덮어쓰는 현상을 줄인다.

표시 문구는 운영 기록의 한국어·영어 요약만 사용하고 서버 절대 경로, IPv4 주소, 토큰·비밀번호·웹훅 형태를 다시 가린다. 명령 원문은 표시하지 않는다. 설정은 사용자 데이터의 `windows-notifications.json`에 최대 64KiB, 스키마 검증, 경로별 프로세스 간 뮤텍스와 원자적 교체로 저장하며 손상·미래 스키마 원본을 보존한다.

UI는 `운영 기록`, `백그라운드`와 트레이에서 진입할 수 있고 기존 현대형 체크박스·드롭다운·그룹·버튼, 다크·라이트 테마, DPI와 스크린 리더 정보를 사용한다. `저장 후 테스트`는 현재 사용자 전용 IPC에서 고정된 안전 문구만 전달한다.

로컬 검증은 기본 비활성화, 설정 왕복, 손상·미래 스키마 보존, 자정 경계 조용한 시간, 중요도·종류 필터, 에이전트 시작 전 기록 재생 방지, 동시 알림 요약, 민감 정보 가림, IPC 테스트 알림과 설정 화면 접근성을 포함한 30개 런처 테스트 그룹을 통과했다. 실제 사용자 알림 설정, 서버, 공유기, UPnP와 외부 포트는 변경하지 않았다.

수정 사항은 `v1.13.0`으로 출시했다. PR과 `main` CI에서 .NET 10 SDK 경고=오류 `net48` 빌드와 전체 검사가 통과했고, 자체서명 릴리스 워크플로도 임시 인증서 제거와 공개 자산 7종을 검증했다. 공개 자산을 별도 폴더에 다시 내려받아 자체서명·SHA-256·메타데이터를 독립 확인했으며, 공개 v1.12.0 런처의 실제 업데이트 코드로 `PUBLIC_AUTO_UPDATE_OK=1.12.0->1.13.0`을 확인한 뒤 업데이트된 실행 파일로 30개 런처 테스트 그룹, 브리지 10개, Portable·UI·보안 검사를 다시 통과했다.

## 29. v1.14.0 실행 중 관리 서버 인계 후속 감사 (2026-07-26)

기존 백그라운드 에이전트는 자신이 시작한 서버만 소유했기 때문에 멀티 서버 관리에서 이미 실행 중인 서버를 유지하려면 창을 계속 열어 두거나 서버를 안전 종료한 뒤 에이전트에서 다시 시작해야 했다. 프로세스 입출력과 소유권을 검증하지 않은 채 기존 Java 프로세스에 붙는 것은 다른 서버 오인 제어, PID 재사용과 로그 유실 위험이 있어 포트나 프로세스 이름만으로 인계하지 않았다.

이번 범위는 이 버전의 멀티 서버 관리가 `--managed-profile`로 시작한 자식으로 한정했다. 자식마다 현재 사용자 SID 전용 난수 이름 파이프와 실행별 256비트 토큰을 만들고, 등록 프로필, 자식 PID·시작 시각, 기존 소유자 PID·시작 시각과 새 에이전트 PID·시작 시각을 모두 검증한 뒤 소유자를 원자적으로 바꾼다. 콘솔 명령은 서버 표준 입력으로만 전달하며 임의 셸이나 실행 파일 경로를 추가하지 않았다.

자식은 제한된 최근 로그 버퍼와 안전한 출력 래퍼를 보유한다. 성공한 인계 뒤 이전 GUI의 리디렉션 출력이 닫혀도 쓰기 오류가 Java 서버 종료로 이어지지 않으며, 에이전트 콘솔에서 인계 전후 로그와 기존 자동완성을 계속 사용할 수 있다. 인계 요청은 멱등하고 응답 유실 시 한 번 재시도한 뒤 실제 새 소유자 상태가 확인될 때만 성공으로 복구한다. 여러 서버 중 일부가 실패하면 창을 닫지 않고 실패 서버의 기존 소유권을 유지하며 성공 서버만 에이전트가 관리한다.

창 닫기 UX는 실행 중 로컬 서버와 에이전트가 함께 있을 때 `중단 없이 인계`, `모두 안전 종료`, `취소`를 구분한다. 인계 중 종료 사건과 추가 창 닫기는 보류해 경쟁 상태에서 세션을 먼저 제거하거나 창을 조기에 닫지 않고, 성공 뒤에만 기존 입출력 래퍼를 정리한다. 실제 에이전트가 사라지면 자식은 기존 부모 감시 정책처럼 Java `stop`을 먼저 보내므로 고아 서버를 무기한 남기지 않는다.

로컬 회귀 검사는 실제 Minecraft 서버 대신 현재 프로세스와 임시 현재 사용자 파이프를 사용한다. 파이프·토큰 형식, 잘못된 토큰, PID 재사용을 나타내는 시작 시각 불일치, 잘못된 기존 소유자, 원자적 소유권 전환, 응답 유실 상태 복구, 중복 인계 멱등성, 명령 단일 전달, 이전 출력 종료 후 로그 유지와 에이전트 상태 등록을 포함한다. 메인 런처 안에서 직접 호스팅하는 Java 서버, 사용자가 직접 시작한 서버와 외부 프로그램 서버의 무중단 인계는 구현된 것으로 표시하지 않는다.

수정 사항은 `v1.14.0`으로 출시했다. PR과 `main` CI에서 .NET 10 SDK 경고=오류 `net48` 빌드와 전체 검사가 통과했고, 자체서명 릴리스 워크플로도 임시 인증서 제거와 공개 자산 7종을 검증했다. 공개 자산을 별도 폴더에 다시 내려받아 자체서명·SHA-256·메타데이터를 독립 확인했으며, 공개 v1.13.0 런처의 실제 업데이트 코드로 `PUBLIC_AUTO_UPDATE_OK=1.13.0->1.14.0`을 확인한 뒤 업데이트된 실행 파일로 31개 런처 테스트 그룹, 브리지 10개, Portable·UI·보안 검사를 다시 통과했다.

## 30. v1.15.0 Discord 원격 제어 베타 후속 감사 (2026-07-26)

Discord 원격 제어는 공개 웹 서버나 웹훅 수신기를 추가하지 않고, 사용자가 별도로 동의한 사용자 계정용 백그라운드 에이전트가 Discord Gateway와 REST API에 나가는 연결을 만드는 방식으로 구현했다. 관리자 권한 서비스, 포트 개방과 특권 Gateway Intent는 사용하지 않는다. 봇 토큰은 현재 Windows 사용자 범위 DPAPI로 암호화하고 평문을 설정·로그·운영 기록에 저장하지 않는다.

길드 전용 `/mineharbor` 명령은 도움말, 허용 서버 상태, 브리지 연결 시 플레이어, 최근 경고·오류, 시작, 백업, 안전 종료와 재시작으로 제한했다. 임의 서버 콘솔, 셸, 실행 파일과 파일 관리는 등록하지도 해석하지도 않는다. 애플리케이션·길드·채널, 허용 사용자 또는 역할, 허용 서버 프로필을 모든 요청에서 확인하고, 사용자별 분당 10회 제한, 상호작용 재생 방지, 응답 길이 상한, 멘션 차단을 적용한다. 외부 소유 프로세스 제어는 기존 백그라운드 에이전트 소유권 검사에서 다시 거부된다.

안전 종료와 재시작은 명령 선택만으로 실행되지 않는다. 암호학적 난수 확인 ID를 같은 사용자·길드·채널·프로필·작업에 묶어 실제 대상과 작업을 표시하고, 60초 안에 한 번만 누를 수 있는 위험 버튼으로 재확인한다. 다른 사용자, 만료, 재사용과 취소는 서버 제어에 도달하지 않는다. Discord 장애나 잘못된 자격 증명은 원격 기능만 중지하며 로컬 서버 제어·예약·백업은 계속 동작한다.

설정 UI는 토큰을 마스킹하고 빈 입력일 때 기존 토큰을 보존하며 별도 체크박스로만 삭제한다. 길드·채널·사용자·역할 ID와 허용 프로필을 분리하고 저장 후 연결 상태를 보여 준다. 기존 테마 입력·체크박스·둥근 표면을 사용하고 한국어·영어, DPI, 키보드 포커스와 스크린 리더 이름·설명을 적용했다.

회귀 검사는 DPAPI 암호문 비저장, 설정 왕복, 손상·미래 스키마 원본 보존, 사용자와 역할 권한, 길드·채널·프로필 범위, 임의 명령 차단, 확인 만료·재사용·다른 사용자 거부, 자동완성 허용 범위, 속도 제한, 응답 상한과 UI 접근성을 포함한다. 테스트는 가짜 상호작용과 임시 설정만 사용하며 실제 Discord 길드, 봇 자격 증명, Minecraft 서버, 공유기와 외부 포트를 변경하지 않는다. 실제 Discord 설치·권한·Gateway 연결은 자격 증명을 가진 별도 시험 길드에서 후속 확인해야 하며, 확인 전에는 베타로 표시한다.

수정 사항은 `v1.15.0`으로 출시했다. PR과 `main` CI에서 .NET 10 SDK 경고=오류 `net48` 빌드와 전체 검사가 통과했고, 자체서명 릴리스 워크플로도 임시 인증서 제거와 공개 자산 7종을 검증했다. 공개 자산을 별도 폴더에 다시 내려받아 자체서명·SHA-256·메타데이터를 독립 확인했으며, 공개 v1.14.0 런처의 실제 업데이트 코드로 `PUBLIC_AUTO_UPDATE_OK=1.14.0->1.15.0`을 확인한 뒤 업데이트된 실행 파일로 32개 런처 테스트 그룹, 브리지 10개, Portable·UI·보안 검사를 다시 통과했다.

## 31. v1.15.1 Discord 첫 연결 UX 후속 감사 (2026-07-26)

기존 Discord 설정 화면은 필요한 보안 경계를 모두 입력할 수 있었지만, 처음 들어온 사용자가 Discord 앱 생성부터 어떤 순서로 준비해야 하는지 화면 안에서 파악하기 어려웠다. 미등록 상태를 보호된 봇 토큰, 유효한 애플리케이션·서버·채널 ID, 허용 사용자 또는 역할과 허용 MineHarbor 프로필이 모두 갖춰졌는지로 판정하고, 처음 메뉴에 진입할 때 4단계 등록 가이드를 먼저 표시하도록 개선했다.

가이드는 기존 테마 팔레트, 둥근 카드와 관리형 버튼을 재사용해 앱·봇 생성, `bot`/`applications.commands` 범위의 길드 설치, 개발자 모드에서 ID 복사, MineHarbor 연결 순서를 한 화면에 배치한다. DPI 변화와 한국어·영어 긴 문구에 대응하며 창 전체 자동 스크롤은 사용하지 않는다. 각 카드와 버튼에 스크린 리더 이름·설명을 넣고 `Enter`는 설정 시작, `Esc`는 설정을 바꾸지 않는 닫기로 연결했다.

가이드 자체는 등록이나 네트워크 요청을 수행하지 않는다. `설정 시작`을 명시적으로 선택한 경우에만 기존 입력 화면으로 이동하고, 이미 등록된 사용자는 바로 설정 화면을 연다. 설정 화면에도 가이드를 다시 여는 보조 버튼을 추가했다. 외부 링크는 고정된 공식 Discord Developer Portal HTTPS 주소 하나뿐이며, 토큰의 현재 사용자 DPAPI 보호와 임의 콘솔·셸·파일 명령 차단을 가이드에서 다시 알린다.

회귀 검사는 기본 미등록 상태와 완전 등록 상태의 분기, 가이드 4개 카드, 세 버튼의 접근성, `Enter`·`Esc` 연결과 설정 화면 재진입 버튼을 기존 Discord 테스트 그룹에 추가했다. 실제 렌더링도 별도 임시 하네스로 확인해 카드 간격, 문구 잘림, 버튼 위계와 불필요한 스크롤이 없음을 점검했으며 사용자 Discord 자격 증명이나 설정은 사용하지 않았다.

수정 사항은 `v1.15.1`로 출시했다. PR과 `main` CI에서 .NET 10 SDK 경고=오류 `net48` 빌드와 전체 검사가 통과했고, 자체서명 릴리스 워크플로도 임시 인증서 제거와 공개 자산 7종을 검증했다. 공개 자산을 별도 폴더에 다시 내려받아 자체서명·SHA-256·메타데이터를 독립 확인했으며, 공개 v1.15.0 런처의 실제 업데이트 코드로 `PUBLIC_AUTO_UPDATE_OK=1.15.0->1.15.1`을 확인한 뒤 업데이트된 실행 파일로 32개 런처 테스트 그룹, 브리지 10개, Portable·UI·보안 검사를 다시 통과했다.

## 32. v1.15.2 Discord 가이드 문구 잘림 후속 감사 (2026-07-26)

실제 다크 테마 화면에서 Discord 등록 단계 설명과 보안 안내가 두 줄로 접힐 때 고정된 25% 행과 76px 보안 행 안에서 아래쪽 문구가 잘리는 문제를 확인했다. 말줄임 설정은 잘림을 해결하지 못하고 필수 설치 안내를 숨길 수 있어 제거했다.

단계 목록은 각 행과 카드·설명 레이블을 내용 기반 자동 높이로 바꾸고, 현재 폭에 맞춘 줄바꿈 너비를 다시 계산한다. 가이드 창도 실제 선호 높이와 모니터 작업 영역을 비교해 초기 높이를 조절한다. 화면이 충분하면 불필요한 스크롤과 빈 공간을 만들지 않고, 부족할 때만 단계 목록 내부에 세로 스크롤을 제공해 헤더·보안 안내·설정 버튼을 유지한다. 세로 스크롤 막대 폭을 미리 제외해 가로 스크롤이 함께 생기지 않게 했다.

사용자 제보와 같은 다크 테마의 실제 WinForms 렌더링에서 네 단계와 보안 문구 전체, 카드 간격, 버튼 위계를 확인했다. 회귀 검사는 네 행의 자동 크기, 설명·보안 레이블의 자동 높이, 말줄임 미사용과 단계 전용 스크롤 컨테이너를 확인한다.

수정 사항은 `v1.15.2`로 출시했다. PR과 `main` CI에서 .NET 10 SDK 경고=오류 `net48` 빌드와 전체 검사가 통과했고, 자체서명 릴리스 워크플로도 임시 인증서 제거와 공개 자산 7종을 검증했다. 공개 자산을 별도 폴더에 다시 내려받아 자체서명·SHA-256·메타데이터를 독립 확인했으며, 공개 v1.15.1 런처의 실제 업데이트 코드로 `PUBLIC_AUTO_UPDATE_OK=1.15.1->1.15.2`를 확인한 뒤 업데이트된 실행 파일로 32개 런처 테스트 그룹, 브리지 10개, Portable·UI·보안 검사를 다시 통과했다.

## 33. v1.15.3 Discord 허용 목록 입력 UX 후속 감사 (2026-07-26)

실제 Discord 설정 화면에서 토큰·애플리케이션·서버·채널의 고정 높이 행이 먼저 공간을 차지하고, 마지막 `허용 사용자`와 `허용 역할` 입력 행이 남은 높이를 백분율로 나누도록 구성돼 있었습니다. DPI 배율과 현재 창 높이에서는 남은 값이 0에 가까워 두 입력칸이 보이지 않고 제목만 남았습니다. 그 결과 사용자는 보안상 필수인 허용 목록을 입력할 방법이 없지만, 저장 후에야 “사용자 또는 역할을 하나 이상 지정”하라는 오류를 받았습니다.

앱 자격 증명 네 항목은 왼쪽 연결 그룹에 유지하고, 허용 사용자·역할은 오른쪽의 허용 프로필과 같은 보안 경계 그룹으로 이동했습니다. 두 다중행 입력은 각각 최소 72px를 보장하고 같은 너비의 두 열로 표시합니다. 사용자 또는 역할 중 하나 이상이 필수라는 문구와 Discord 개발자 모드에서 사용자 ID를 복사하는 경로를 입력 바로 아래에 배치했습니다. 남는 세로 공간은 별도 비어 있는 백분율 행이 흡수하므로 마지막 채널 입력이 과도하게 늘어나지 않습니다.

필수 값 누락과 Discord ID 형식 오류는 저장 전에 하단 인라인 상태로 갱신합니다. 사용자가 저장을 선택해도 예상 가능한 입력 오류에는 일반 오류 대화상자를 먼저 띄우지 않고 해당 토큰·ID·허용 목록·프로필로 키보드 포커스를 이동합니다. 상태 레이블과 새 배치는 한국어·영어, 다크·라이트 팔레트와 스크린 리더 이름을 사용합니다. 폼 핸들이 만들어지기 전 프로필 체크 변경이나 닫힌 뒤 예약된 검증 콜백도 안전하게 건너뜁니다.

실제 다크 테마 WinForms 렌더링에서 두 허용 목록 입력, ID 복사 안내, 즉시 누락 상태와 하단 동작이 한 화면에 표시되는지 확인했습니다. 이 과정에서 발견한 마지막 채널 입력의 과도한 확장과 긴 체크박스 문구도 함께 정리했습니다. 자동 회귀 검사는 허용 목록의 두 열 배치·최소 높이·접근성, 누락 시 정확한 포커스 대상, 유효 사용자 ID 입력 뒤 연결 준비와 마지막 고정 입력 뒤 여백 행을 검사합니다. 실제 Discord 자격 증명, 길드, 사용자 설정과 네트워크 연결은 사용하지 않았습니다.
