...
## v1.7.1 후속 조치 (2026-07-22)

- 보조 창은 내부 배경만 팔레트로 바꾸고 Windows 11 제목 표시줄·외곽 테두리와 일부 네이티브 스크롤바에는 테마를 전달하지 않아 다크 모드에서 밝은 틀이 남았습니다.
- 공통 DWM 테마 바인딩을 모든 보조 창 경로에 적용하고, 핸들 재생성 시에도 현재 팔레트를 다시 사용하도록 수정했습니다. RichTextBox·목록·표·숫자 입력 등 네이티브 컨트롤도 핸들 생성 시점까지 다크 테마 상태를 보존합니다.
- 입력용 RoundedPanel의 역할을 구분해 배경과 테두리를 함께 맞추고, 빠른 명령 관리의 기본 GroupBox는 둥근 ModernGroupBox로 교체했습니다. 콘솔의 기본 외곽선과 검색·명령 입력 표면을 정돈하고 관리 콘솔 출력이 상·하단 도구 영역 뒤로 겹치지 않게 했습니다.
- 회귀 테스트는 다크 보조 창·리치 텍스트·목록·체크 목록·입력/그룹 테두리·표 헤더를 실제 팔레트 값으로 검증합니다. 실제 공유기 설정이나 사용자 서버 데이터는 변경하지 않았습니다.
...
## 19. v1.7.3 반응형 UX 후속 감사 (2026-07-24)

Windows 11의 실제 125% DPI 환경에서 메인 화면을 한국어·영어, 다크·라이트, 콘솔 닫힘·열림 조합으로 다시 확인했다. 서버 시작, 명령 전송, 파일 설치·삭제와 UPnP는 실행하지 않았으며 언어와 테마도 점검 뒤 한국어·다크로 복원했다.

확인된 문제는 영어 런처 업데이트 버튼, 빠른 명령 상태·안내·관리 버튼의 말줄임표, 콘솔을 닫았을 때 남는 큰 빈 공간, 콘솔 도구막대의 역순 배치와 `Word wrap` 잘림이었다. 빠른 명령 카드는 콘솔 닫힘 시 전체 폭, 열림 시 360~460 논리 픽셀 범위의 보조 패널로 전환하도록 수정했다. 상태와 안내는 고정 말줄임표를 제거하고 실제 문자열 폭으로 버튼을 계산한다. 콘솔 도구막대는 검색 → 로그 분류 → 줄 바꿈 순서와 충분한 양언어 폭을 사용한다.

키보드 흐름에서는 시작 준비 완료 후 주 동작인 서버 시작으로 포커스를 옮겼다. 기존 빠른 명령·관리 콘솔·플레이어 이름 자동완성에 더해 메인 콘솔은 기본 명령과 현재 브리지가 제공한 플레이어 인수를, 예약 명령 편집은 실행 시점과 무관한 기본 명령을 제안한다. 후보는 위아래 방향키, Tab/Enter, Esc로 조작하며 접근성 이름과 설명을 제공한다.

회귀 테스트는 제한된 폭으로 측정해 잘림을 놓치던 기존 검사를 수정해 4096픽셀의 제한 없는 실제 문구 폭을 사용한다. 콘솔 닫힘·열림의 Dock 전환, 상태·안내의 말줄임표 부재, 양언어 버튼 폭, 콘솔 도킹 순서, 줄 바꿈 문구 폭과 자동완성 연결을 별도 그룹으로 검증해 `PASSED=26`이 됐다. 125% 실화면은 확인했지만 150%·200%, 혼합 DPI 다중 모니터, Windows 고대비와 Narrator의 실제 음성 순서는 격리 장치 행렬의 후속 항목으로 남는다.

## 20. v1.7.4 닫기 입력과 종료 확인 후속 감사 (2026-07-25)

보조 창의 제목 표시줄 닫기 버튼 위에서 발생한 추가 클릭이 뒤쪽 메인 창 버튼에 전달될 가능성을 다시 점검했다. v1.7.3에서 한 번의 일반적인 물리 클릭으로는 현상을 재현하지 못했지만, 창이 닫힌 직후 `BeginInvoke`에서 입력 보호를 즉시 해제하던 구현은 마우스 더블 클릭이나 하드웨어 클릭 채터링의 두 번째 입력을 막지 못했다.

수정본은 닫기 버튼의 화면 좌표를 기억하고 Windows 더블 클릭 시간에 150ms를 더한 500~1,200ms 동안 그 좌표 주변 64픽셀에서만 메인 창의 마우스 입력을 무시한다. 따라서 겹친 버튼의 우발적 실행은 차단하면서, 사용자가 보호 영역 밖을 의도적으로 클릭하는 동작은 즉시 허용한다. 보호 시간 만료 전·후와 경계 좌표를 회귀 테스트로 고정했다.

메인 런처는 서버가 꺼져 있고 작업이 없는 경우에도 종료 여부를 묻는다. 이 일반 종료 경로는 서버 종료 프로세스를 실행하지 않고 사용자가 확인하면 바로 창을 닫는다. 진행 중인 작업은 작업 완료 후 종료한다는 문구를, 실행 중인 서버는 월드를 저장하고 서버를 안전하게 종료한 뒤 닫는다는 문구를 한국어와 영어로 각각 제공하며 기존 안전 종료 절차를 유지한다.

실제 Windows 환경에서 서버를 시작하거나 네트워크·UPnP 상태를 변경하지 않은 채 한국어 다크 테마의 일반 종료 확인 창을 확인했다. 자동 회귀 검증은 런처 테스트 26개 그룹, 브리지 프로토콜 10개 사례, Portable 버전·실행 검사, 모던 대화상자와 보안 정적 검사를 포함한다.

수정 사항은 `v1.7.4`로 출시했다. PR과 `main` CI, 자체서명 릴리스 산출물 7종 검사가 모두 통과했으며, 공개 v1.7.3 런처가 게시된 메타데이터를 통해 v1.7.4로 업데이트된 뒤 동일한 전체 회귀 검사를 다시 통과했다.

## 21. v1.7.5 빠른 명령·콘솔 작업 영역 후속 감사 (2026-07-25)

v1.7.3에서 빠른 명령을 콘솔 닫힘 시 전체 폭, 열림 시 오른쪽 360~460px 패널로 전환하도록 바꿨으나, 사용자가 콘솔을 열고 닫을 때 카드 위치가 달라지는 문제가 있었다. 실제 v1.7.4 창의 경계를 측정한 결과 콘솔은 작업 영역 전체 폭을 유지하고 빠른 명령 패널이 그 위에 겹쳐, 콘솔 오른쪽 출력과 입력 일부가 가려지는 현상도 확인했다.

작업 영역을 2열 `TableLayoutPanel`로 교체해 콘솔을 왼쪽 가변 열, 빠른 명령을 구버전과 같은 오른쪽 410px 고정 열에 배치했다. 콘솔 표시 여부는 각 열의 위치와 폭을 바꾸지 않으며, 앞뒤 순서나 `BringToFront`에 의존하지 않는다.

회귀 검사는 고정 열의 위치·폭, 콘솔 전환 전후 위치 유지, 콘솔과 빠른 명령의 별도 셀, 실제 컨트롤 경계 비중첩을 검증한다. Windows 다크 화면에서도 닫힘 → 열림 → 닫힘 순서로 확인해 빠른 명령이 오른쪽에 유지되고 콘솔 도구막대·출력·명령 입력이 가려지지 않음을 확인했다. 서버 시작·명령 전송·UPnP는 수행하지 않았다. Portable 전체 회귀와 자체서명 릴리스 산출물 7종 검증도 통과했다.

수정 사항은 `v1.7.5`로 출시했다. PR과 `main` CI, 자체서명 릴리스 산출물 7종 검사가 모두 통과했으며, 공개 v1.7.4 런처가 게시된 메타데이터를 통해 v1.7.5로 업데이트된 뒤 동일한 전체 회귀 검사를 다시 통과했다.

## 22. v1.8.0 Paper/Purpur 복사 동작 설정 및 전체 UI 후속 감사 (2026-07-25)

Paper 공식 문서를 기준으로 피스톤 복사(`unsupported-settings.allow-piston-duplication`), 엔드 포털 중력 블록 복사(`unsupported-settings.allow-unsafe-end-portal-teleportation`), 철사 덫 갈고리 복사(`unsupported-settings.skip-tripwire-hook-placement-validation`)를 서버별로 설정할 수 있게 했다. Paper/Purpur 1.19 이상은 `config/paper-global.yml`, 이전 버전은 `paper.yml`의 `settings.unsupported-settings`를 사용한다. 중력 블록은 1.20.4 이상, 철사덫은 1.21.4 이상 또는 실제 생성된 키를 확인한 경우에만 활성화한다. 직접 JAR은 기존 Paper 설정과 감지된 키 범위만 지원하며, Spigot·Vanilla 및 비 Paper 계열에는 Paper 전용 키를 기록하지 않는다.

설정 쓰기는 서버 루트 밖 경로와 재분석 지점을 거부하고, 탭 들여쓰기·중복 `unsupported-settings` 섹션·중복 관리 키·잘못된 불리언 값이 있는 파일을 수정하지 않는다. 실제 내용이 바뀔 때만 같은 서버의 `.mineharbor/configuration-backups`에 백업을 만들고 파일별 최신 5개만 보존한다. 기존 프로필은 설정 창에서 명시적으로 저장하기 전까지 관리 기능이 비활성화되어 수동 YAML을 덮어쓰지 않는다.

UI 감사에서는 설정 창의 규칙 설명과 고급 옵션 사이에 남아 있던 146px 공백을 제거하고 새 호환 설정 그룹을 배치했다. 정상 창 크기에서는 스크롤이 필요하지 않으며, 좁은 높이에서만 세로 스크롤을 사용한다. 저장소 전체를 대상으로 기본 WinForms `Button`, `ComboBox`, `CheckBox`, `TrackBar`, 표준 스크롤바 사용을 거부하고 모든 22개 폼이 DPI 배율을 선언하는지 검사하도록 정적 회귀 검사를 확장했다. 체크박스와 상태 문구에는 한·영 지역화, 키보드 포커스, 스크린 리더 이름·설명을 적용했다.

검증은 Paper 최신·구형 경로, 직접 JAR 감지, Spigot 거부, 설정 보존, 중복 섹션 거부, 백업·무변경 재실행을 포함한다. 실제 사용자 서버, 포트, UPnP 매핑, 공유기 및 사용 중인 Windows 화면은 변경하지 않고 임시 디렉터리와 리플렉션 기반 UI 검사만 사용했다.

## 23. v1.8.1 보조 창 닫기 해제 입력 후속 감사 (2026-07-25)

v1.8.0 공개 자동 업데이트는 성공했지만, 업데이트된 실행 파일로 전체 회귀를 반복하던 중 사용자가 실제 마우스를 움직이자 보조 창 닫기 입력 관통 검사가 한 차례 실패했다. 닫기 누름 시점의 화면 좌표에서 64px 이상 이동하면 같은 물리 입력의 마우스 해제까지 좌표 필터에서 제외되는 것이 원인이었다.

닫기 누름과 짝인 해제 이벤트는 위치와 무관하게 한 번 소비하도록 바꿨다. 해제는 단독으로 버튼 클릭을 만들지 않으며, 주 창에 전달할 이유가 없으므로 포인터 이동 중에도 막는 편이 안전하다. 그 뒤의 추가 누름은 기존처럼 닫기 좌표 주변에서만 짧게 차단해 다른 위치의 의도적인 클릭은 유지한다. 회귀 테스트는 닫기와 해제 사이에 포인터 상태를 반경 밖으로 옮겨 이 동작을 결정적으로 검증한다.

수정 사항은 `v1.8.1`로 출시했다. 핫픽스 PR과 `main` CI, 자체서명 릴리스 산출물 7종 검사가 모두 통과했으며, 공개 v1.8.0 런처가 게시된 메타데이터를 통해 v1.8.1로 업데이트된 뒤 27개 테스트 그룹, Portable 실행·버전, 명령 브리지 10개, UI·보안 회귀 검사를 다시 통과했다.
